...
// cell_holder lets you know which image type was set for the cell for the
// cell_click function.
cell_holder = "";
var nrow = 13;
var ncol = 13;
...
function draw_grid (nrow, ncol) {
    ...
    // create an array with each entry being the number of bombs around that
    // entry
    // set all default values to false, because null doesn't work
    for (i=0; i<nrow; i++) {
        for (j=0; j<ncol; j++) {
...
function rand (min, max) {
    ...
    // % note 1: See the commented out code below for a version which will work
    // with our experimental (though probably unnecessary) srand() function)
    // * example 1: rand(1, 1);
    // * returns 1: 1
    ...
    if (!game_over) {
        if (event.button == "0") {
            ...
            if (is_game_over) {
                ...
                // start: debug //
                // $(e).css("background-image", bomb).css("background-repeat",
                // "no-repeat").css("background-position", "50% 50%");
                // end: debug //
            }
        }
...
function clear_cells (i, j) {
    ...
    if (a_is_open[i][j] == false) {
        ...
        // also make sure to check if it's a border block to not check any
        // negative margins, or margins greater than nrow or ncol
        if (a_bombs[i][j] == true) {
            game_over = true;
            show_bombs();
...
function apply_css_default_style () {
    $("div.cell").css("border", "2px solid #404040").css("background-color", "gray").css("width", "35px").css("height", "35px").css("text-align", "center").css("float", "left").css("vertical-align", "middle").css("display", "table-cell");
